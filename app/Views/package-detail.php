...
<div class="page-content header-clear-medium">
    <?php if($data['totalResults'] > 0): ?>
        <div class="card card-style">
            <div class="content">
                <div class="d-flex align-items-center justify-content-between gap-3">
                    <h1 class="font-700 pb-0">Paket #<?= $data['uid'] ?></h1>
        ...
        <div class="card card-style">
            <div class="content">
                <?= form_open('/dashboard/paket/' . $data['uid'] . '/edit') ?>
                    <?= csrf_field() ?>
                    <div class="form-custom form-label form-icon mb-3">
                    	<i class="bi bi-box-seam font-14"></i>
                    	<input type="text" class="form-control rounded-xs" id="name" name="name" placeholder="Nama paket" value="<?= $data['result'][0]['name'] ?>" required="">
...
<div class="offcanvas offcanvas-modal rounded-m offcanvas-detached bg-theme" style="width: 340px;visibility: hidden;" id="menu-warning" aria-hidden="true">
	<div class="gradient-red px-3 py-3">
		...
		<a href="/dashboard/paket/<?= $data['uid'] ?>/hapus" class="default-link btn btn-full btn-s bg-white color-black">Ya, lanjutkan</a>
	</div>
</div>
<?= $this->endSection() ?>