...
<div class="page-content header-clear-medium">
    <?php if($data['totalResults'] > 0): ?>
        <div class="card card-style">
            <div class="content">
                <div class="d-flex align-items-center justify-content-between gap-3">
                    <h1 class="font-700 pb-0">Sales #<?= $data['uid'] ?></h1>
        ...
        <div class="card card-style">
            <div class="content">
                <?= form_open('/dashboard/sales/' . $data['uid'] . '/edit') ?>
                    <?= csrf_field() ?>
                    <div class="form-custom form-label form-icon mb-3">
                    	<i class="bi bi-person-circle font-14"></i>
                    	<input type="text" class="form-control rounded-xs" id="name" name="name" placeholder="Nama Sales" value="<?= $data['result'][0]['name'] ?>" required="">
                    	<label for="c1" class="color-theme">Nama</label>
                    </div>
                    <div class="form-custom form-label form-icon mb-3">
                    	<i class="bi bi-envelope font-14"></i>
                    	<input type="text" class="form-control rounded-xs" id="email" name="email" placeholder="Email Sales" value="<?= $data['result'][0]['email'] ?>" required="">
                    ...
                    <div class="form-custom form-label form-icon mb-3">
                    	...
                    	<input type="text" class="form-control rounded-xs" id="password" name="password" placeholder="Password" value="<?= $data['result'][0]['password'] ?>" required="">
...
<div class="offcanvas offcanvas-modal rounded-m offcanvas-detached bg-theme" style="width: 340px;visibility: hidden;" id="menu-warning" aria-hidden="true">
	<div class="gradient-red px-3 py-3">
		...
		<p class="color-white opacity-60 pt-2">
			Apakah anda yakin akan menghapus Sales?
		</p>
		<a href="/dashboard/sales/<?= $data['uid'] ?>/hapus" class="default-link btn btn-full btn-s bg-white color-black">Ya, lanjutkan</a>
	</div>
</div>
<?= $this->endSection() ?>